...
// Port configuration
const port = process.env.PORT || config.PORT || 8889;

// Create the server and listen on the specified port
const express = require('express');
const app = express();

// Additional code to define your routes and middlewares
app.get('/', (req, res) => {
...
app.listen(port, (err) => {
    if (err) {
        console.error(`Port ${port} par server start karne me error:`, err);
        process.exit(1); // Error hone par process exit kar jaayegi
    } else {
        console.log(`Server port ${port} par listening kar raha hai`);
    }
});
